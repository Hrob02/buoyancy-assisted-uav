...
sortedTable = sortrows(decisionMatrix, 'rank_SA_V');
...
title('Disturbance Stability Index');
grid on;
...
sgtitle('Envelope Geometry Metric Comparison (Lower Is Better Per Metric)');
...
data = [rankingRobustness.percentage_ranked_first_SA_V, ...
    rankingRobustness.percentage_ranked_first_disturbance, ...
    rankingRobustness.percentage_non_pareto_dominated];
bar(data);
set(gca, 'XTick', 1:height(rankingRobustness), 'XTickLabel', cellstr(rankingRobustness.shape));
ylabel('Sensitivity-case share [%]');
legend({'Ranked first (SA/V)', 'Ranked first (disturbance)', 'Non-Pareto-dominated'}, 'Location', 'northeast');
title('Sensitivity Metric Ranking Robustness');
grid on;
safe_export_figure(figureHandle, fullfile(cfg.paths.figures_dir, 'envelope_sensitivity_metric_ranking.png'));
close(figureHandle);
end
